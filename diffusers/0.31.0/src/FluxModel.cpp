...
std::tuple<Tensor, Tensor> get_1d_rotary_pos_embed(
    int dim,
    Tensor pos,
    bool repeat_interleave_real,
    bool use_real,
    Tensor::ScalarType freqs_dtype
) {
    assert(dim % 2 == 0 && "Dimension must be divisible by 2");
    
    // 计算 scale
    Tensor scale = Tensor::arange(0, dim, 2, freqs_dtype, pos.device());
    scale = scale / dim;
    
    // 计算 omega
    float theta_float = static_cast<float>(theta_);
    Tensor omega = 1.0f / torch::pow(theta_float, scale);
    
    // 计算 out
    Tensor out = torch::einsum("...n,d->...nd", {pos.to(freqs_dtype), omega});
    
    // 计算 cos 和 sin
    Tensor cos_out = torch::cos(out);
    Tensor sin_out = torch::sin(out);
    
    if (repeat_interleave_real) {
        cos_out = cos_out.repeat_interleave(2, -1);
        sin_out = sin_out.repeat_interleave(2, -1);
    }
    
    if (use_real) {
        cos_out = cos_out.reshape({pos.shape[0], -1, dim / 2, 1});
        sin_out = sin_out.reshape({pos.shape[0], -1, dim / 2, 1});
    }
    
    return std::make_tuple(cos_out, sin_out);
}

FluxPosEmbed::FluxPosEmbed(int theta, const std::vector<int>& axes_dim) 
    : theta_(theta), axes_dim_(axes_dim) {}

std::tuple<Tensor, Tensor> FluxPosEmbed::forward(Tensor ids) {
    const int n_axes = ids.shape[-1];
    std::vector<Tensor> cos_out;
    std::vector<Tensor> sin_out;
    
    // 转换为浮点类型
    Tensor pos = ids.to(Tensor::FP32);
    ...
    for (int i = 0; i < n_axes; i++) {
        Tensor pos_slice = pos.slice(-1, i, i + 1);
    ...
    // 在最后一个维度上拼接结果
    Tensor freqs_cos = Tensor::cat(cos_out, -2);
    Tensor freqs_sin = Tensor::cat(sin_out, -2);
    
    return std::make_tuple(freqs_cos, freqs_sin);
}