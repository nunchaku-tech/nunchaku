...
class OminiFluxDownsizeAllInOnePipeline(FluxPipeline):
    ...
    def _encode_vae_image(
        self, images: torch.Tensor, generator: torch.Generator = None
    ):
        if isinstance(generator, list):
            image_latents = [
                self.vae.encode(images[i : i + 1]).latent_dist.sample(generator[i])
    ...
    def _prepare_latent_image_ids(batch_size, height, width, device, dtype):
        ...
        latent_image_ids[..., 1] = (
            latent_image_ids[..., 1] + torch.arange(0, height)[:, None]
        )
        latent_image_ids[..., 2] = (
    ...
    def _prepare_latent_image_sub_ids(
        batch_size, height, width, device, dtype, step=1, w_offset=0, h_offset=0
    ):
        latent_image_ids = torch.zeros(height, width, 3)
        latent_image_ids[..., 1] = (
            latent_image_ids[..., 1]
            + torch.arange(0, height * step, step)[:, None]
            + h_offset
        )
        latent_image_ids[..., 2] = (
            latent_image_ids[..., 2]
            + torch.arange(0, width * step, step)[None, :]
            + w_offset
        )
    ...
    def prepare_sub_image_latents(
        ...
        latent_image_ids.append(
            self._prepare_latent_image_sub_ids(
                batch_size, height // 2, width // 2, device, dtype, step=2
            )
        )
        latent_image_ids.append(
            self._prepare_latent_image_sub_ids(
                batch_size, height // 2, width // 2, device, dtype, step=2, h_offset=1
            )
        )
        latent_image_ids.append(
            self._prepare_latent_image_sub_ids(
        ...
        latent_image_ids.append(
            self._prepare_latent_image_sub_ids(
                batch_size, height // 2, width // 2, device, dtype, step=2, w_offset=1
            )
        )
    ...
    def __call__(
        ...
        generator: Optional[Union[torch.Generator, List[torch.Generator]]] = None, # Random generator for reproducibility
        ...
        if downsize:
            condition_image = self.image_processor.preprocess(
                image, height=height // 2, width=width // 2
            )
        else:
            condition_image = self.image_processor.preprocess(
