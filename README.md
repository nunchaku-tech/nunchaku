...
2. Install `nunchaku` package:
	...
	# TMPDIR=/root/autodl-tmp/pip-temp VERBOSE=1 PIP_CACHE_DIR=~/autodl-tmp/.cache/pip  MAX_JOBS=6 pip install -v  --no-deps --no-build-isolation -e .


	MAX_JOBS=6 pip install --no-deps --no-build-isolation -e .
	MAX_JOBS=6 pip install --no-deps --no-build-isolation -e . --no-compile
